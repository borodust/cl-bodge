...
(defun apply-pretext-scissors (cmd poiu)
  (let ((width (width-of poiu))
    ...
    (flet ((clamp (v max)
             #f(max (min v max) 0)))
      (dissect-c-struct ((x :x) (y :y) (w :w) (h :h)) (cmd %nk:command-scissor)
        (gl:scissor (clamp x width) (clamp (- height y h) height)
                    (clamp w width) (clamp h height))))))
    ...
    (with-canvas ((canvas-of poiu) (floor (width-of poiu)) (floor (height-of poiu)))
      (bodge-nuklear:docommands (cmd (handle-value-of poiu))
        (case (command-type cmd)
          (:nop)
          (:scissor (render-scissor cmd) (setf last-scissor cmd))
          ...
          (:text (when last-scissor (push last-scissor commands)) (push cmd commands))

...
(defmethod initialize-instance :after ((this nvg-image) &key image canvas)
  ...
  (with-slots (context id data) this
    (let ((ctx (handle-value-of canvas))
      ...
      (setf context ctx
            id (nvg:make-rgba-image ctx (width-of image) (height-of image)
                                    (foreign-pointer-of data)
                                    :generate-mipmaps :repeatx :repeaty)))))


(defun image->nvg (canvas image)
  (make-instance 'nvg-image :image image :canvas canvas))
